...
int eFBCTunerManager::setProcFBCID(int fe_id, int fbc_id)
{
	...
	if(fd < 0)
	{
		eDebug("[eFBCTunerManager] setProcFBCID failed to open %s: %m", filename);
		return -1;
	}
	else
	{
...
{
	return (fe_slot_id%8 < m_fbc_tuner_num);
}
...
int eFBCTunerManager::connectLink(eDVBRegisteredFrontend *link_fe, eDVBRegisteredFrontend *prev_fe,
	eDVBRegisteredFrontend *next_fe, bool simulate)
{
...
int eFBCTunerManager::disconnectLink(eDVBRegisteredFrontend *link_fe, eDVBRegisteredFrontend *prev_fe,
	eDVBRegisteredFrontend *next_fe, bool simulate)
{
...
void eFBCTunerManager::connectSortedLink(eDVBRegisteredFrontend *link_fe,
	eDVBRegisteredFrontend *top_fe, bool simulate)
{
...
{
	eFecDebug("[eFBCTunerManager] addLink: %p(%d)->%p(%d) %s", top_fe, FE_SLOT_ID(top_fe), link_fe, FE_SLOT_ID(link_fe), simulate ? "(simulate)" : "");
