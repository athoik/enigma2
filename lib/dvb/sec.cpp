		...
		if ( sit != lnb_param.m_satellites.end())
		{
			...
			else
			{
				switch(lnb_param.SatCR_format)
				{
					case 1:
					{
						eDebug("[eDVBSatelliteEquipmentControl] [prepare] JESS");

						int tmp1 = abs(sat.frequency
							-lof)
							- 100000;
						int tmp2 = (1000 + 2 * tmp1) / (2 *1000); //round to multiple of 1000
						frequency = lnb_param.SatCRvco - (tmp1 - (1000 * tmp2));
						frontend.setData(eDVBFrontend::FREQ_OFFSET, sat.frequency - lof -(lnb_param.SatCRvco - frequency));
						...
						eDebug("[eDVBSatelliteEquipmentControl] [prepare] UnicableTuningWord %#06x, guard_offset %d",lnb_param.UnicableTuningWord ,lnb_param.guard_offset);
						frontend.setData(eDVBFrontend::FREQ_OFFSET, lnb_param.SatCRvco); //FIXME
					}
					break;
					case 0:
					default:
					{
						eDebug("[eDVBSatelliteEquipmentControl] [prepare] Unicable");
						...
						int tmp2 = (4000 + 2 * tmp1) / (2 * 4000); //round to multiple of 4000
